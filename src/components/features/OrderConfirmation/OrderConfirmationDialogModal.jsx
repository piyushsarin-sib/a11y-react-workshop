...
/**
 * OrderConfirmationDialogModal component using DialogOverlay
 * Displays order confirmation and feedback form with accessible modal
 */
const OrderConfirmationDialogModal = ({ dialogState, onClose }) => {
  const [showAnimation, setShowAnimation] = useState(true);

  // Automatically transition from animation to confirmation after 4 seconds
  useEffect(() => {
    if (dialogState.body.visible && showAnimation) {
  ...
  }, [dialogState.body.visible, showAnimation]);

  // Reset animation when dialog opens
  useEffect(() => {
    if (dialogState.body.visible) {
      setShowAnimation(true);
    }
  }, [dialogState.body.visible]);
  ...
  // Custom close handler that calls onClose callback
  const handleClose = () => {
    if (onClose) {
      onClose();
    } else {
...
export default OrderConfirmationDialogModal;
